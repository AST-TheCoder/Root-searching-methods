...
for i in range(100):
    id=perm%10
    if id==0:
        ans1=(p-b*ans2-c*ans3)/a
    if id==1:
        ans2=(p-a*ans1-c*ans3)/b
    if id==2:
        ans3=(p-a*ans1-b*ans2)/c

    id=int(perm/10)%10
    if id==0:
        ans1=(q-v*ans2-w*ans3)/u
    if id==1:
        ans2=(q-u*ans1-w*ans3)/v
    if id==2:
        ans3=(q-u*ans1-v*ans2)/w

    id=int(perm/100)%10
    if id==0:
        ans1=(r-y*ans2-z*ans3)/x
    if id==1:
        ans2=(r-x*ans1-z*ans3)/y
    if id==2:
        ans3=(r-x*ans1-y*ans2)/z
